...
def main(snvphyl_version_settings, galaxy_url, galaxy_api_key, deploy_docker, copy_fastq_files_to_docker, docker_port, docker_cpus, docker_other_options, with_docker_sudo, keep_deployed_docker, snvphyl_version, workflow_id, fastq_dir, fastq_files_as_links, fastq_history_name, reference_file, run_name,
         relative_snv_abundance, min_coverage, min_mean_mapping, repeat_minimum_length, repeat_minimum_pid, disable_density_filter, filter_density_window, filter_density_threshold, invalid_positions_file, output_dir):
    """
    ...
    if (deploy_docker and (galaxy_url or galaxy_api_key)):
        ...
        try:
            main_galaxy(url, key, snvphyl_version, workflow_id, fastq_dir, fastq_history_name, reference_file, run_name, relative_snv_abundance, min_coverage, min_mean_mapping,
                repeat_minimum_length, repeat_minimum_pid, disable_density_filter, filter_density_window, filter_density_threshold, invalid_positions_file, output_dir)
        finally:
...
def main_galaxy(galaxy_url, galaxy_api_key, snvphyl_version, workflow_id, fastq_dir, fastq_history_name, reference_file, run_name, relative_snv_abundance, min_coverage, min_mean_mapping,
    ...
    if LooseVersion(snvphyl_version) >= LooseVersion('1.2.0'):
        set_parameter_value(workflow_settings,workflow_parameters,'use-density-filter', not disable_density_filter)
    elif disable_density_filter:
...
if __name__ == '__main__':
    ...
    parameter_group.add_argument('--disable-density-filter', action="store_true", dest="disable_density_filter", default=False, required=False, help='Disable SNV density filtering')
